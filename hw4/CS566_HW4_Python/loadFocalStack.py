...
def load_focal_stack(focal_stack_dir):
    ...
    if not files:
        raise FileNotFoundError(f"No images found in '{focal_stack_dir}'. Put your focal stack images there.")
    files = sorted(files)  # ensure consistent front-to-back order
    ...
    for i, f in enumerate(files[1:], start=1):
        ...
        if im_rgb.shape[:2] != (H, W):
            raise ValueError(f"All images must have identical size. '{f}' has shape {im_rgb.shape[:2]}, expected {(H,W)}.")
        rgb_stack[..., i] = im_rgb
